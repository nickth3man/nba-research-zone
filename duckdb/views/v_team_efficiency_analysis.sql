...
WITH team_efficiency AS (
    ...
    FROM sqlite_db.team t
    JOIN sqlite_db.season s ON t.season_id = s.season_id
    LEFT JOIN sqlite_db.game g ON (g.home_team_id = t.team_id OR g.away_team_id = t.team_id)
        AND g.season_id = t.season_id
        AND g.game_type = 'Regular Season'
    LEFT JOIN sqlite_db.team_season_advanced tsa ON t.team_id = tsa.team_id
        AND t.season_id = tsa.season_id
    LEFT JOIN sqlite_db.team_game_other_stats tos ON t.team_id = tos.team_id
