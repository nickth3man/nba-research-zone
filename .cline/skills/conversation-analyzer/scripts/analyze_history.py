...
def extract_patterns(conversations):
    ...
    for conv in conversations:
        ...
        if any(kw in display for kw in ["fix", "error", "bug", "issue", "problem"]):
            ...
            kw in display for kw in ["explain", "what", "how", "why", "analyze", "understand"]
        ):
    ...
    for task, occurrences in task_groups.items():
        if len(occurrences) > 3:
            mistakes["repeated_requests"].append(
                {"pattern": occurrences[0][1], "count": len(occurrences)}
            )
...
def generate_report(patterns, mistakes):
    ...
    if mistakes["repeated_requests"]:
        report.append(
            f"### 3. REPEATED EXACT REQUESTS ({len(mistakes['repeated_requests'])} patterns)"
        )
        report.append("Exact same requests multiple times - could be automated:")
        for req in sorted(mistakes["repeated_requests"], key=lambda x: x["count"], reverse=True)[
            :5
        ]:
    ...
    if mistakes["multi_step_without_planning"]:
        report.append(
            f"### 5. COMPLEX MULTI-STEP REQUESTS ({len(mistakes['multi_step_without_planning'])} found)"
        )
...
def generate_recommendations(patterns, mistakes):
    ...
    error_ratio = sum(patterns["error_keywords"].values()) / max(
        sum(patterns["request_types"].values()), 1
    )
    if error_ratio > 0.3:
        recommendations.append(f"{rec_num}. REDUCE ERROR RATE")
        recommendations.append(
            f"   Problem: High error rate detected ({error_ratio * 100:.1f}% of requests)"
        )
        recommendations.append("   Solutions:")
